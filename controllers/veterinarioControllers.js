...
const olvidePassword = async (req, res) =>{
    ...
    const existeVeterianrio = await Veterinario.findOne({ email });
    if(!existeVeterianrio){
        const error = new Error('El Usuario no existe');
        return res.status(400).json({msg: error.message});
    }

    try {
        existeVeterianrio.token = generarId()
        await existeVeterianrio.save()

        //enviar email con las intrucciones
        emailOlvidePassword({
            email,
            nombre: existeVeterianrio.nombre,
            token: existeVeterianrio.token
        })
...
const  nuevoPassword = async (req,res)=>{
    ...
    try {
        ...
        res.json({msg: 'El password modificado correctamente'});
    } catch (error) {
        console.log(error)
    }
};
...
const actualizarPassword = async (req,res) => {
    ...
    //comprobar su pssword
    if(await veterinario.comprobarPassword(pwd_actual)){
         //almecenar el nuevo password
